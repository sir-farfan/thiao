...
'''
tsub.py - Used instead of sbatch to submit jobs, it will additionally create the
...
if __name__ == '__main__':
    ...
    try:
        ...
        job = Thiao.job_submit(sys.argv[1])    
    print ("got this job id: ", job)
    print ("retrieving job requirements")
    nodes, memory = Thiao.get_job_requirements(job)
    print ("nodes: ", nodes)
    vms = Thiao.list_down_vms()
    if len(vms) >= nodes:
        print ("launching VMs for job")
        vms = vms[:nodes]
        Thiao.launch_vm(vms, job)
    else:
